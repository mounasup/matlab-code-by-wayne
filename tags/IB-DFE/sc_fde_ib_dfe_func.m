% File: sc_fde_ib_dfe_func.m
% --------------------------
% This script implements SC-FDE-IB-DFE function based on 'sc_fde_ib_dfe.m'
% and [1][2].

function [BER ber_mfb] = sc_fde_ib_dfe_func(pd_in, nfft_in, snr_in, ntrial)
%%
...
% path_delays_norm = (0: 1: 4);
path_delays_norm = pd_in;
...
% length_pload = nfft-length_cp; % #of pay load symbols in null UW scheme
...
%             uw = [xtx(p); uw(1: end-1)]; % ideal feedback for null UW scheme
...
%             xib_ext = [x_ib;zeros(length_cp,1)]; % for null UW scheme
%             only
...
title('QPSK-SC-IB-DFE over Rayleigh Fading Channel');
ylim([5e-6 1]);
grid on
%%
% 
% filename = 'sc_fde_ib_dfe.mat';
