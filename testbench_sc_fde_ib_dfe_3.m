% File: testbench_sc_fde_ib_dfe_3.m
% ---------------------------------
...
%             rs = Sd'*XTX/(sqrt(M_Sd)*sqrt(M_XTX)*nfft)
...
%                 C(q) = M_XTX*H(q)'/(nfft*noisePower+abs(H(q)).^2*(1-abs(rs).^2)*M_XTX);
...
%                 B(q) = -(rs*(H(q)*C(q)-gamma))/(sqrt(M_Sd)*sqrt(M_XTX));
...
% End of script
% [1] N. Benvenuto and S. Tomasin, ¡°Iterative design and detection of a DFE
% in the frequency domain,¡± Communications, IEEE Transactions on,  vol. 53, 2005, pp. 1867-1875.