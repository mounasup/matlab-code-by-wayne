% File: sc_fde_ib_dfe_rho.m
% -------------------------
% This script implements SC-FDE-IB-DFE system with rho estimation, based 
% on 'sc_fde_ib_dfe.m' and [1][2].
close all
clear all
...
length_pload = nfft; % #of pay load symbols in CP scheme
...
SNR = 6:2:22; % Range of SNR values, in dB. receiver snr
...
%         x = randint(nfft,nblock,M_mod);
        x = randi([0,M_mod-1], length_pload, 1);
...
%                                          % with the pay load.
        XTX = fft(xtx_pload,nfft); % DFT of transmitted symbols, used at rx end for filter design.
...
%             uw = [xtx(p); uw(1: end-1)]; % ideal feedback
            uw = [xtx_pload(p); uw(1: end-1)]; % ideal feedback for CP scheme
        end
        %%
...
%            xib_ext = [x_ib;zeros(length_cp,1)];
            xib_ext = x_ib; % for CP scheme only
...
% Plot BER results.
figure();
semilogy(SNR,BER(1,:),'-*',SNR,BER(2,:),'-o',SNR,BER(3,:),'-p',SNR,BER(4,:),'-s',SNR,ber_mfb,'rd');
legend('ZF','MMSE','DFE','IB','MFB');
xlabel('SNR (dB)'); ylabel('BER');
title('QPSK-SC-IB-FDE over Rayleigh Fading Channel');
ylim([5e-6 1]);
grid on
%%

filename = 'sc_fde_ib_dfe_rho.mat';
save(filename, 'SNR', 'BER', 'ber_mfb', 'path_delays_norm', 'doppler_max', 'pdp');

% End of script
% [1] N. Benvenuto and S. Tomasin, ¡°Iterative design and detection of a DFE in the frequency domain,¡± Communications, IEEE Transactions on,  vol. 53, 2005, pp. 1867-1875.
% [2] A. Chan and G. Wornell, ¡°A class of block-iterative equalizers for intersymbol interference channels: fixed channel results,¡± Communications, IEEE Transactions on,  vol. 49, 2001, pp. 1966-1976.
